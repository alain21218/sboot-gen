...
const generateRepo = input => {
    ...
    fs.writeFile(url, repo(params.name, src.package), err => {
        if (err) throw err;
...
const generateEntity = input => {
    ...
    fs.writeFile(url, entity(params.name, src.package), err => {
        if (err) throw err;
...
const generateRest = input => {
    ...
    fs.writeFile(url, ctrl(params.name, src.package), err => {
        if (err) throw err;
...
const createSwaggerConfig = input => {
    return new Promise((resolve) => {
        ...
        fs.writeFile(url, swaggerConfig(src.package), err => {
            if (err) throw err;
